...
namespace tac
{
  ...
  const char* textTitle = "Magic Revolver";
  ...
  void menu_t::OnClick()
  {
    ...
    for (uint32_t i = 0; i < menu_t::total; ++i)
    {
  ...
  void menu_t::Prepare()
  {
    ...
    auto centerPos = glm::vec3(
      CenterTextOffset(textTitle, 20.0f*aspect),
      2.0f,
      0.0f
    );

    this->title = bb::GenerateMesh(
      bb::DefineNumber(centerPos, 0.15f, glm::vec2(1.0f), textTitle)
    );
