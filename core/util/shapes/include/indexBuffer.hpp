...
namespace bb
{
  ...
  int indexBuffer_t<data_t>::Append(const basicIndexBuffer_t &src, size_t offset)
  {
    ...
    for (auto cursor = itemsToAppend, end = itemsToAppend + itemSize; cursor != end; ++cursor)
    {
      ...
      size_t newIndex = cursor[0] + offset;
      ...
      this->self.emplace_back(
        static_cast<data_t>(newIndex & std::numeric_limits<data_t>::max())
      );
    }
    return 0;
  }
  ...
  template <>
  GLenum indexBuffer_t<uint8_t>::Type() const;
  template <>
  GLenum indexBuffer_t<int8_t>::Type() const;
  template <>
  GLenum indexBuffer_t<uint16_t>::Type() const;
  template <>
  GLenum indexBuffer_t<int16_t>::Type() const;
  template <>
  GLenum indexBuffer_t<uint32_t>::Type() const;
  template <>
  GLenum indexBuffer_t<int32_t>::Type() const;
