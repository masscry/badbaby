...
namespace bb
{
  ...
  void font_t::Load(const std::string& filename)
  {
    ...
    this->texture = std::shared_ptr<texture_t>(new texture_t(texture_t::LoadTGA(fontConfig.Value("font.texture", "").c_str())));

    const auto size = bb::vec2_t(1.0f/this->width, 1.0f/this->height);
    ...
    for (uint8_t smb = 0; smb < std::numeric_limits<uint8_t>::max(); ++smb)
    {
      ...
      this->offsets[output[0]] = vec2_t { smbPos.x * size.x, smbPos.y * size.y };
  ...
  {
    std::vector<uint32_t> symbols = utf8extract(text.c_str());
